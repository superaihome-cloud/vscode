...
import type { URI, Snapshot, SessionConfigSchema, SessionSummary, ModelSelection, Turn, TerminalClaim, SessionActiveClient, MessageAttachment, ContentRef, StringOrMarkdown } from './state.js';
...
export type ChangesetOperationTarget =
	| { kind: 'resource'; resource: URI; side?: 'before' | 'after' }
	| { kind: 'range'; resource: URI; side?: 'before' | 'after'; range: { start: number; end: number } };

/**
 * Optional follow-up surfaced by the server after an operation completes —
 * typically a URI to open (e.g. a created PR) or a {@link ContentRef} the
 * client can fetch.
 *
 * @category Commands
 */
export type ChangesetOperationFollowUp =
	| { kind: 'openUri'; uri: URI }
	| { kind: 'content'; content: ContentRef };
